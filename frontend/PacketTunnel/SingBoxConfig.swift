...
enum SingBoxConfig {
        ...
        let config: [String: Any] = [
            ...
            "dns": [
                "servers": [
                    ...
                    // Российские домены — системным резолвером телефона. DoH к 1.1.1.1
                    // напрямую из РФ душится DPI → резолв «через раз»; local быстро и
                    // надёжно резолвит .ru-сервисы.
    ...
    private static func makeProxyOutbounds(_ proxy: ProxyConfig) -> [[String: Any]] {
        ...
        for (i, ep) in endpoints.enumerated() {
            ...
            let epSNI = ep.serverName ?? proxy.serverName ?? ""
            outs.append(realityOutbound(proxy, host: ep.host, port: ep.port, serverName: epSNI, tag: tag))
            tags.append(tag)
        }
        outs.append([
            "type": "urltest",
